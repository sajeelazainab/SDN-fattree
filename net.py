...
class Mininet( object ):
    ...
    def iperf_single(self, hosts=None, udpBW='10M', period=60, port=5501):
        if not hosts:
            return
        else:
            assert len(hosts) == 2
        ...
        output('***Iperf:testing bandwith between')
        output("%s and %s\n" % (client.name, server.name))
        iperfArgs = 'iperf -u'
        bwArgs = '-b ' + udpBW + ' '
        print"***start server***"
        server.cmd(iperfArgs + '-s -i 1' + '>/home/mininet/test' + filename + '&')
        print "***start client***"
        client.cmd(iperfArgs + '-t' + str(
            period) + '-c' + server.IP() + ' ' + bwArgs + '>/home/mininet/test' + 'cilent' + filename +'&')
